...
//leveldb中实现的简单的hash table，采用定长数组存放node，链表解决hash冲突，每次instert后，
//如果node数量大于数组的容量，就容量扩大为2倍，做一次rehash
class HandleTable 
{
 public:
 ...
 private:
  ...
  LRUHandle** FindPointer(const Slice& key, uint32_t hash) 
  {
	...
	{
      ptr = &(*ptr)->next_hash;	 //注：指向的是next_hash指针的地址
    }
    return ptr;
  }
	...
	void (*deleter)(const Slice& key, void* value)) 
{
  ...
  //最近最少使用 当usage大于capacity时候，删除最近最少使用那个
