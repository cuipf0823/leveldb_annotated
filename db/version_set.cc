...
namespace leveldb 
{
//compact过程中，level-0中的sstable由memtable直接dump生成，不做大小的限制，非level-0中的sstable的大小设定为kTargetFileSize
...
static double MaxBytesForLevel(int level)
{
  ...
  double result = 10 * 1048576.0;  // Result for both level-0 and level-1 10 * 1024 * 1024
  while (level > 1) 
  {
    result *= 10;
    level--;
...
Status Version::Get(const ReadOptions& options, const LookupKey& k, std::string* value, GetStats* stats) 
{
  ...
  for (int level = 0; level < config::kNumLevels; level++)
  {
	...
	// 等效于FileMetaData* const* files = files_[level]
...
Status VersionSet::Recover(bool *save_manifest) 
{
  ...
  Builder builder(this, current_);
  {
    ...
    std::string scratch;
	/*
		从manifast文件中依次读取record，检查comparator是否一致
		manifast文件和log文件格式完全一样
	*/
    while (reader.ReadRecord(&record, &scratch) && s.ok())
	{
	  ...
	  /*
	     检测解析MANIFEST的最终状态中的基本信息是否完整（log_number， SequenceNumber， FileNumber），将其生成db当前状态
		 包含db的各种元信息（FileNumber，SequenceNumber， 各个level的文件数目，size，range下次compact的start-key）都
		 load成功，db恢复到上一次退出前状态
	  */
      if (s.ok()) 
	  {
        builder.Apply(&edit);
...
Compaction* VersionSet::PickCompaction()
{
  ...
  // compaction_score_ >= 1说明compaction_level_上最不均衡
  ...
  // 当level == 0的时候，由于其中的sstable会有overlap 取出在level-0中与已经确定的compact的sstable有overlap的文件
  if (level == 0) 
  {
...
Compaction::Compaction(int level)
    : level_(level),
      max_output_file_size_(MaxFileSizeForLevel(level)),	  //2mb
      input_version_(NULL),
...
//compact时，当key的valuetype是kTypeDeletion时，要检查level-n+1以上是否存在，来确定是否丢弃该key
//因为key的遍历是顺序的，所以每次检查从上一次检查结束的地方开始
//level_ptrs_[i]中记录了input_version_->levels_[i]中，上次比较结束的sstable的容器下标
