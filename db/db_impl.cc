...
Status DBImpl::Recover(VersionEdit* edit, bool *save_manifest)
{
  ...
  // 根据传入的db路径，对LOCK文件做flock，判断是否已经有db启动实例
  // lock文件不存在，创建它
  env_->CreateDir(dbname_);
  assert(db_lock_ == NULL);
  Status s = env_->LockFile(LockFileName(dbname_), &db_lock_);	 //lock文件
  if (!s.ok())
  {
    return s;
  }

  if (!env_->FileExists(CurrentFileName(dbname_))) 	            //current文件
  {
	//默认false
  ...
  //从MANIFEST文件中依次读取每一个record，检查Comparator是否一致，然后依次replay
  //这里save_manifest==false
  ...
  std::vector<std::string> filenames;
  s = env_->GetChildren(dbname_, &filenames); //获取dbname_路径下全部子文件
  if (!s.ok()) 
  {
    return s;
  }
  ...
  //根据log文件的FileNumber和遍历record的SequenceNumber修正当前的FileNumber和SequenceNumber
                              ...
                              SequenceNumber* max_sequence)
{
  ...
  while (reader.ReadRecord(&record, &scratch) && status.ok()) 
  {
	...
	//write_buffer_size 4MB	  memtable达到阈值（write_buffer_size）dump成sstable
...
void DBImpl::BackgroundCompaction() 
{
  ...
  */
  ...
  {
	//不是manual compact 且选出的sstable都处于level-n不会造成过多的
	//grandparentoverlap 简单处理将这些sstable推到level-n+1 更新db元信息即可
...
WriteBatch* DBImpl::BuildBatchGroup(Writer** last_writer)
{
  ...
  size_t max_size = 1 << 20;//（1024 * 1024）
  if (size <= (128<<10))
  {
...
Status DBImpl::MakeRoomForWrite(bool force) 
 {
  ...
  while (true) 
  {
	...
	else if (allow_delay && versions_->NumLevelFiles(0) >= config::kL0_SlowdownWritesTrigger) //kL0_SlowdownWritesTrigger == 8
	{
	  //level-0中的文件数超过了8 sleep，delay一次
	...
	{
      // There is room in current memtable 未达到write_buffer_size允许这次写
      break;
    } 
	else if (imm_ != NULL)
	{
	  //达到了阈值，但是immutable memtable仍然存在，则等待compact将其dump完成；
	...
	{
      // There are too many level-0 files. level-0层文件数达到了12个，等待compact
      Log(options_.info_log, "Too many L0 files; waiting...\n");
      bg_cv_.Wait();
    } 
	else 
	{
	  //memtable已经写满，但是immutable memtable不存在，将当前memtable转换成immutable，生成新的logfile 和 memtable 主动触发compact
